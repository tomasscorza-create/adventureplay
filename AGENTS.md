...
- En Desafio, las plataformas calculan una paleta complementaria a partir del color medio de cada fondo. Las capas de sombra, cara inferior, textura y borde son solo visuales: no deben modificar ni duplicar sus cuerpos Arcade.
...
- `src/assets/scenery/ancient-trials/`: seis fondos WebP de Desafio ordenados desde noche hasta tesoro, mas dos laminas fuente para mecanismos. `PreloadScene` extrae en runtime cajas, placas, palancas, puertas, plataformas y barreras eliminando el fondo blanco; no volver a mostrar las laminas completas dentro de gameplay.
...
- `src/game/systems/net/CoopSession.ts`: capa de transporte co-op (singleton fuera de Phaser) sobre Supabase Realtime; canal por sala con broadcast + presence, key de presence por clientId unico, validacion de `COOP_PROTOCOL_VERSION` entre peers y modelo de slots (`localSlot`, `participants`, `characterIdForSlot`).
- `src/game/systems/net/coopMessages.ts`: contrato de red del co-op (input con slot emisor en bitmask, `NetPlayerState`, `players` del snapshot como arreglo por slot, roster `assignSlots`, codigos de sala, version de protocolo, payload de presence). Cambiar la forma de estos datos exige subir `COOP_PROTOCOL_VERSION`; `coopMessages.test.ts` congela los bits del input y la asignacion de slots.
...
- `src/game/systems/net/CoopProjectilePuppets.ts`: sprites sin fisica que representan en el guest los proyectiles autoritativos del host (crea al aparecer un `netId`, interpola y destella al desaparecer).
- `src/ui/screens/CoopExitConfirmScreen.tsx`: confirmacion de salida en co-op; la escena no se pausa mientras esta visible.
...
- Jugar co-op online de dos jugadores tanto en Desafio como en Explorar. El lobby `CoopLobby.tsx` (parametrizado por modo) crea o une salas por codigo de 4 caracteres; el transporte vive en `src/game/systems/net/` sobre Supabase Realtime (broadcast + presence, canal `coop-room-<CODE>`, sin tablas ni RLS). El modelo es host-autoritativo: el host simula toda la fisica (su heroe en slot A, el del guest en slot B) y transmite snapshots a ~20 Hz; el guest envia su input al cambiar (mas keepalive), fundiendo los flancos `justPressed` en los bits para que los taps tactiles de un solo frame no se pierdan, congela los cuerpos autoritativos y renderiza el snapshot con interpolacion. Los proyectiles de poder letal viajan en el snapshot con `netId` y el guest los representa como sprites sin fisica (`CoopProjectilePuppets`), con destello al desaparecer. Cada dispositivo tiene camara propia sobre su personaje. El fin es compartido: cualquier derrota o victoria la decide el host y aplica a ambos. En Explorar, la linea de presion co-op sigue al jugador mas atrasado y no usa checkpoints. Pausar en co-op no pausa la escena: abre la confirmacion React `coop-exit-confirm` (pantalla de `GameScreen`) con la partida corriendo detras; salir avisa al peer via `GO_TO_MENU`. El modo red solo se activa cuando `START_GAME` trae `coop`; single-player queda identico.
...
13. Escalar el co-op a mas de dos jugadores en fases. Hecho: generalizacion del protocolo (input con slot emisor, `players` como arreglo por slot, roster `assignSlots`, `COOP_PROTOCOL_VERSION = 4`). Pendiente: gameplay y lobby para 3-4 jugadores (reemplazar `player`/`player2` fijos por un arreglo, cooldowns/cargas por slot, presion sobre el mas atrasado de N, subir `COOP_MAX_PLAYERS`), y por ultimo costo de red (delta-encoding, cuotas de Supabase Realtime), reconexion y pausa co-op acordada. Mantener el transporte fuera de Phaser y el plumbing compartido en `CoopSceneLink`.
...
- El co-op online funciona con dos jugadores. El protocolo y la sesion ya usan un modelo de slots N-ready (input con slot emisor, `players` como arreglo por slot, roster determinista con `assignSlots`, `COOP_PROTOCOL_VERSION = 4`), pero las escenas todavia instancian `player`/`player2` fijos (slot 0/1) y `COOP_MAX_PLAYERS = 2`. Renderizar 3-4 jugadores es la fase siguiente. Verificarlo exige dos clientes con cuentas distintas; los recorridos co-op son manuales.
